...
$\epsilon$은 Gaussian noise 이다. 여기서 DDPM의 저자들은 $\mathbf{\mu}_\theta$ 를 $\frac{1}{\sqrt{\alpha_t}}\left( \mathbf{x}_t - \frac{\beta_t}{\sqrt{1-\bar{\alpha}_t}} \epsilon _\theta(\mathbf{x}_t, t) \right)$ 로 parameterize 했는데, 이는 $p$와 $q$의 mean term을 서로 비슷하게 만들어주기 위함이다. 그러면 결과적으로 아래와 같은 최종 loss function을 얻을 수 있다(\ref{eq9}). 

...
이때 $\bar{\mathbf{p}}_{i}$는 pixel $i$의 screen space position, $\bar{\mathbf{f}}_{i,k}$는 $k$번째 scene feature, $\alpha_i^2, \beta_i^2, \gamma_{k,i}^2$는 각각 $i$의 spatial, color, $k^\text{th}$ feature에 대한 variance, $D$, $D_k$는 color와 feature간의 거리를 측정하기 위한 특수한 함수들이다. 결국 $\alpha_i, \beta_i, \gamma_{k,i}$에 따라 $d_{i,j}$가 바뀌기 때문에, 이들의 값을 잘 예측하는 것이 관건이고, 이전의 feature-based filter을 사용하는 연구들은 모두 이 변수들의 값을 예측하는 방식에 차이가 있다. 
